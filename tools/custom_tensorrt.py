...
def parse_args():
    ...
    parser.add_argument('--p', help='dir for pretrained model', default='/workspace/pretrained_models/cityscapes/PIDNet_S_Cityscapes_test.pt', type=str)
    parser.add_argument('--r', help='root or dir for input images', default='/workspace/samples/', type=str)
...
if __name__ == '__main__':
    ...
    print(images_list)
    sv_path = '/workspace/samples/outputs/tensorrt_outputs/'
    """ 
    ...
    #summary(module, input_size=(1, 3, 1024, 2048))
    trt_model_fp32 = torch_tensorrt.compile(module, inputs=[torch_tensorrt.Input((1, 3, 1024, 2048))],  enabled_precisions = torch.float32, workspace_size = 1 << 22)
    torch.jit.save(trt_model_fp32, "trt_ts_module.ts")
    """
    model = torch.jit.load('trt_ts_module.ts')
    print('done reading')
    with torch.no_grad():
        print('torch_no')
